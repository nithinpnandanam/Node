...
* if there are 10 clients requesting data from the server then 10 socket connections are formed
...
---
* Refer Image 3
* while accesing a website using a domain name at first we send a request to DNS **Domain Name System**
...
---
* Refer Image 4
* I can create multiple http server corresponding to an ip address which can be differentiated using ```port number```
...
---
* Refer Image 6
* we can host the FE application and BE application on same server or different server
...
* generally when api calls are made socket connection is used
* web sockets are resource intensive
* when 4,5 web sockets are open it consumes more resources i.e, more load to the server
---
